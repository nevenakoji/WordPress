<?php
if ( ! class_exists( 'SBTTB_Plugin_Installer' ) ) {
    class SBTTB_Plugin_Installer {
        ...
        public function after_plugin_active() {
            if ( isset( $_GET['sbttb-hide-notice'] ) && isset( $_GET['_sbttb_nonce'] ) && $_GET['sbttb-hide-notice'] == 'install-variation-price-plugin' && wp_verify_nonce( $_GET['_sbttb_nonce'], 'install-variation-price-plugin' ) ) {
                set_transient( 'sbttb_variation_price_plugin_notice', 'yes', 2 * MONTH_IN_SECONDS );
                update_option( 'sbttb_variation_price_plugin_notice', 'yes' );
            }
        }
        ...
        public function internal_feed() {
            ...
            if ( current_user_can( 'install_plugins' ) && $screen && in_array( $screen->id, $visible_pages ) ) {
                ...
                if ( apply_filters( 'stop_sbttb_live_feed', false ) ) {
                    return;
                }

                if ( get_option( 'sbttb_variation_price_plugin_notice' ) == 'yes' ) {
                    return;
                }

                if ( get_transient( 'sbttb_variation_price_plugin_notice' ) == 'yes' ) {
                    return;
                }
                ...
                $button_text = esc_html__( 'Install Now', 'smooth-back-to-top-button' );
                ...
                if ( in_array( $plugin_slug, $plugins ) ) {
                    $button_text = esc_html( 'Activate Plugin', 'smooth-back-to-top-button' );
                ...
                $cancel_url = esc_url(
                    add_query_arg(
                        array(
                            'sbttb-hide-notice' => 'install-variation-price-plugin',
                            '_sbttb_nonce'      => wp_create_nonce( 'install-variation-price-plugin' ),
                        )
                    )
                );

                $image_url = SBTTB_DIR_URI . 'admin/assets/img/wvpd-icon.gif';

                echo sprintf( '<div class="sbttb-feed-contents notice notice-info" style="position: relative; clear: both; overflow: hidden; padding: 15px;"><div class="sbttb-feed-message-wrap"><image src="%s" style="float: left; margin-right: 15px;"/><p style="padding: 0; margin: 0 0 10px;">5000+ WooCommerce stores increase their sales using <a target="_blank" class="thickbox open-plugin-details-modal" href="%s"><strong>WooCommerce Variation Price Display</strong></a>. Why not yours? <br/> This plugin helps you to show the variable product price in a more user-friendly way to your customers.</p> <a class="button-primary" href="%s" rel="noopener">%s</a></div>
                    <a class="sbttb-live-feed-close notice-dismiss" href="%s" style="text-decoration: none; opacity: .5;"></a></div>', $image_url, $popup_url, $install_url, $button_text, $cancel_url );
